...
public class CallExpressionNode extends ExpressionNode {
    ...
    public CallExpressionNode copy() {
        ...
        for (int i = 0; i < arguments.length; i++) {
            if (arguments[i] == null)
                continue; //Possible because when doing something like script1(script2()) where script1 has 2 args and script2 returns 2 values, only 1 item will be on the stack!
